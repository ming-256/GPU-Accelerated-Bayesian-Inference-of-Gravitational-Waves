"""
Phase-Marginalized (Unheterodyned) Nested Sampling for GW170817 (Kazewong/Bilby data)
=====================================================================================

Same inference pipeline as GW170817_unheterodyned_1.py but uses pre-processed
bilby data from EventData/kazewong/ instead of fetching from GWOSC. This
evaluates the full frequency-domain inner product over all bins (no relative
binning approximation) — useful as a reference run to validate heterodyning
with kazewong data.

Key differences from GW170817_unheterodyned_1.py:
...
Key differences from GW170817_heterodyned_kazewong.py:
  - No heterodyning (full likelihood, no relative binning)
  - No reference parameters needed
  - Chunked init to avoid OOM on A100 40GB

Usage:
  python GW170817_unheterodyned_kazewong.py [--waveform {IMRPhenomD_NRTidalv2,TaylorF2}]
"""
...
# ============================================================================
parser = argparse.ArgumentParser(description='Unheterodyned nested sampling for GW170817 (kazewong data)')
...
gps = 1187008882.43
fmin = 23.0
fmax = 1792.0    # Match kazewong reference (not 2048)
duration = 128
post_trigger_duration = 2

waveform_tag = args.waveform
label = f'Results/PhaseMarg_Unheterodyned_Kazewong_{waveform_tag}_psd-{psd_source}'
...
# which for 2000 × ~226k freq bins × 3 dets × complex128 can exceed GPU memory.
INIT_CHUNK = 100
...
print("JIT-compiling first step (this will be slow — ~226k freq bins)...")
t_jit0 = time.time()
...
print(f"\n{'='*50}")
print(f"TIMING SUMMARY (UNHETERODYNED, KAZEWONG DATA)")
print(f"{'='*50}")
