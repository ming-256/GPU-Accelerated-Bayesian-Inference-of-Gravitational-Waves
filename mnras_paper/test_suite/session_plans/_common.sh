...
TEST_SUITE_ROOT="${REPO_ROOT}/mnras_paper/test_suite"
...
mkdir -p "${RESULTS_ROOT}" "${TEST_SUITE_ROOT}/logs"
...
init_session() {
    ...
    SESSION_LOG="${TEST_SUITE_ROOT}/logs/${SESSION_TAG}.${SESSION_START_EPOCH}.log"
...
JSON
    ...
    if [[ ${exit_status} -eq 0 ]]; then
        ${PYTHON} "${TEST_SUITE_ROOT}/scripts/_update_catalog_status.py" "${run_id}" done "${CATALOG}" || true
    else
        ${PYTHON} "${TEST_SUITE_ROOT}/scripts/_update_catalog_status.py" "${run_id}" failed "${CATALOG}" || true
    fi
}
